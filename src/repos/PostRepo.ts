...
export class PostRepo extends Repository<Post> {
  ...
  static async getPostsByUserId(userId: number): Promise<NotificationDTO[]> {
    const entityManager = getManager()
    const query =
      'SELECT DISTINCT pic.postId, n.alertId, n.postId , pic.url, n.creationDate, l.lat, l.long ' +
...
function buildWhereStatements(pet: Pet): string {
  let query = ''
  if (pet.furLength != undefined && pet.furLength.Id!=undefined) {
    if (query.length < 5) {
      query = query + ' WHERE'
    } else {
      query = query + ' AND '
  ...
  if (pet.color !== undefined&&pet.furLength.Id!=undefined) {
    if (query.length < 5) {
      query = query + ' WHERE'
    } else {
      query = query + ' AND '
  ...
  if (pet.breed !== undefined&&pet.breed.Id!=undefined) {
    if (query.length < 5) {
      query = query + ' WHERE'
    } else {
      query = query + ' AND '
  ...
  if (pet.size !== undefined&&pet.size.Id!=undefined) {
    if (query.length < 5) {
      query = query + ' WHERE'
    } else {
      query = query + ' AND '
